...
1. **先索引后选择，禁止凭记忆/历史快照硬编码 ID**
   - 每次分析先调数据源 skill 的目录/搜索接口（如 `dashboards/search`）拉完整索引
   - 按业务关键词从返回值的分组名 + 名称 + description 里筛选候选
   - 拿到的 dashboard/panel ID 再调 details 接口确认变量名和口径
   - 历史 panel_id 可作业务关键词参考，**不能跳过 search 直接用**

2. **数据不足时报警手动转 SQL，不自动 spawn 取数 subagent**
   - 触发条件（命中任一就停下报警）：
     - 索引里没匹到对应业务的看板
     - 看板存在但口径与需求差太多（缺关键维度切分/过滤条件）
     - 需要事件级原始数据或自定义聚合
     - 需要跨多事件 union / 复杂 JOIN 的逻辑
   - 报警格式：列出已检索看板（名称+ID）+ 缺什么（具体维度/过滤口径/指标）+ 建议 SQL 方向，让用户手动启动取数 agent 跑自定义 SQL，CSV 落盘后再继续分析
   - 不要硬塞相似看板，不要自动 spawn 取数 subagent —— 自定义取数的口径需要分析师参与确认，自动化会绕过这层把关导致结果跑偏
...
2. **数值列写裸数值，单位放列名**：单元格只允许 number / 百分号字符串 / `+`/`-` 开头的差异格 这种特殊符号。**禁止把单位写进单元格**（"1.8台"、"42.9min"、"126次"、"≈0" 都不行）。
