...
> 当报告涉及**多个时间窗口**（如 V1→V2→V3→V4→V5 的版本演进、多个时段对比）或**多个对比对象**（如多分群、多业务线、多产品）时，强制使用本节 6 条规则。这些规则来自实际报告迭代中反复返工总结，跳过任意一条都可能导致用户来回拉锯。

### 1. 表格 ↔ 结论 方向相反规则

| 维度 | 方向 | 原因 |
|------|------|------|
| **表格行序**（上→下）| 最新 → 最旧 | 一眼看到最新值，符合 "latest-snapshot 优先" 的快速阅读 |
| **结论叙述方向**（左→右）| 最旧 → 最新 | `X → Y → Z` 与 "先发生→后发生" 语义一致，便于阅读演进 |

- 用户首次给到时间序列任务时，**先口头确认这两条方向**（多数用户会沿用此默认）
- 表格脚注里写明："表格按最新→最旧排，结论按时间正向叙述"

### 2. 结论 bullet 标准格式

```
{指标名} {V1} → {V2} → {V3} → {V4} → {V5}（变化率/百分点）；{业务定性词}
```

- **数据先行**：所有窗口完整罗列。**窗口数 ≥3 时禁止只给首末两端**（中段波动会被隐藏，让"逐步下降"之类的定性词失真）；窗口数 = 2（如版本更新前/后对比）时 `A → B` 本身就是完整数据，正常写。
- **变化率挂尾**：百分比指标用 `pp`（如 `-12.4pp`）、绝对量/货币用 `%`（如 `+34.5%`）
- **业务定性挂在数据之后**：末尾追加"逐步下降 / 基本稳定 / 略有提升"等定性词，但需配合规则 5 校准
- **反例**：`X 指标同步下行` ❌（无数据）/ 5 窗口报告里写 `X 指标 48.8% → 36.4%` ❌（5 个窗口只挑两端，藏掉中段波动）
- **正例（5 窗口）**：`X 指标 48.8% → 38.1% → 41.7% → 34.7% → 36.4%；逐步下降` ✅
- **正例（2 窗口）**：`X 指标 48.8% → 36.4%（-12.4pp）；更新后下行` ✅

### 3. 数值精度统一

| 类型 | 格式 | 示例 |
|------|------|------|
| 百分比 | 1 位小数 | `48.8%`（**禁止 `48.79%`**）|
| 货币 | 货币符号前缀 + 1 位小数 | `￥0.4`、`$1.7` |
| 时长 | `XX.X min` | `55.3min` |
| 大数 | 千分位 | `26,150` |
| 百分点差 | `±X.Xpp` 文本 | `+6.9pp`、`-12.4pp` |
| 相对变化率 | `±X.X%` | `+34.5%`、`-65.0%` |

> 注：内嵌 sheet 表格里百分比差异格因 formatter 限制只能 2 位小数（`100.60%`），但**结论叙述文本**仍按 1 位小数写。

### 4. 按对象分章节 + 收尾横向对比

- 对比多个对象（分群 / 版本 / 活动 / 模式…）时，**优先按对象逐个出聚合表 + 趋势结论**（一对象一章节），不要按维度横向铺开成混合表（混合表通常 >9 列触发规则 6 的拆分约束，且不利于聚焦单个对象的演进）
- 最后**强制追加**「横向对比总结」章节：
  - 行 = 对象，列 = 各维度的变化幅度
  - 单元格用 `±X%` / `±Xpp`，触发飞书内嵌 sheet 自动着色（绿涨红降）
  - 描述 bullets 按维度组织（量级 / 留存 / 活跃质量 / 付费 / 偏好…），每条点出"谁最大 / 谁最小 / 谁逆势"

### 5. 自动趋势词必须用户校准

基于首末值 ±5% / ±15% 阈值的自动趋势判断在多窗口形态下**不可靠**：

- 反例 A：`18% → 25% → 25% → 25% → 27%` 端点 `+52%` 自动判定 "逐步提升"，实际后 4 窗口稳定，应为 "**基本稳定**"
- 反例 B：`32% → 25% → 38% → 35% → 37%` 端点 `+15%` 自动判定 "提升"，但中段大幅波动，应为 "**基本稳定**"

**强制流程**：模式偏好 / 留存 / 活跃质量等需要看完整形态的指标，初稿用自动 `trend_word()` 起草后，**主动让用户核对全 N 窗口形态再固化定性词**。横向对比章节的"谁分化 / 谁同向"判断同样需要用户校准。

### 6. 单章节子表拆分（>9 列时强制）

- 一个对象的指标维度过多导致单表 ≥10 列时，**强制拆**为主表 + `charts[]` 子表
  - 主表 `chart_data`：核心维度（如活跃 + 付费 = 7 列）
  - 子表 `charts[0..N]`：辅助维度（如留存全档 6 列、模式参与 7 列）
- 单表硬上 ≥10 列易触发飞书 sheet API 静默失败（`addSheet` 异常 / 单元格写入丢失），且阅读时左右滚动也降低可用性
- 拆分后每张子表自带独立 `title` + `footnote`，沿用规则 1-3
